...
const modules = props.items ? {} : (import.meta as any).glob(['../../p/**/*.md', '../../p/*.md'], { eager: true })
...
const catalogTree = computed(() => {
  ...
  const allPages = Object.entries(modules).map(([path, mod]: [string, any]) => {
    ...
    return {
      path: vitePressPath,
      title: mod.frontmatter?.title || mod.__pageData?.title || vitePressPath.split('/').pop()?.replace('.md', ''),
  ...
  function buildSubTree(dir: string, currentLevel: number): any[] {
    ...
    const files = allPages.filter(f => f.dir === dir && !f.path.endsWith('index.md'))
      .sort((a, b) => {
    ...
    const subDirPaths = [...new Set(allPages
...
<template>
  <div :class="['catalog', { 'recursive': depth > 0 }]" :style="{ marginLeft: depth > 0 ? '1.25rem' : '0' }">
        ...
        <div v-if="item.type === 'file'" class="catalog-item">
          <a :href="resolveLink(item.path)" class="catalog-link">
            <div class="title-wrap">
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="octicon">
                <path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z" />
                <path d="M14 2v4a2 2 0 0 0 2 2h4" />
              </svg>
              <span class="catalog-title">{{ item.title }}</span>
            </div>
            <span v-if="item.frontmatter?.date" class="catalog-date">{{ item.frontmatter.date }}</span>
          </a>
        </div>
        <div v-else class="catalog-subdir">
          <div class="subdir-header">
            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="octicon-dir">
              <path d="M20 20a2 2 0 0 0 2-2V8a2 2 0 0 0-2-2h-7.9a2 2 0 0 1-1.69-.9L9.6 3.9A2 2 0 0 0 7.93 3H4a2 2 0 0 0-2 2v13a2 2 0 0 0 2 2Z" />
            </svg>
            <component :is="item.indexPath ? 'a' : 'span'" :href="item.indexPath ? resolveLink(item.indexPath) : null" class="subdir-title" :class="{ 'is-link': item.indexPath }">
              {{ item.title }}
            </component>
          </div>
    ...
    <div v-if="depth === 0 && catalogTree.length === 0" class="catalog-empty">
      暂无文档
    </div>
  </div>
</template>
